...
"  Last Modified: 2009-11-02 09:30
"        Version: 1.63
"=============================================================================
...
if g:QFixHowm_Default_Key > 0
  ...
  exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'rc :<C-u>call QFixHowmCreateNewFileWithFilename()<CR>'
...
"  exec "autocmd BufEnter     *.".g:QFixHowm_FileExt." nmap <silent> <buffer> <expr> <CR> QFixHowmActionLock()"
  ...
  autocmd BufWinEnter quickfix call <SID>QFixHowmSetup()
  autocmd BufWinLeave quickfix call QFixHowmSaveMru(-1)
augroup END

function! s:QFixHowmSetup()
  if g:QFix_MyJump == 0
    return
  endif
  ...
  nnoremap <buffer> <silent> D  :call QFixHowmCmd_D()<CR>
  vnoremap <buffer> <silent> D  :call QFixHowmCmd_D()<CR>
...
"  vnoremap <buffer> <silent> X  :call QFixHowmCmd_X('move')<CR>
  ...
  nnoremap <buffer> <silent> R  :call QFixHowmCmd_R()<CR>
  vnoremap <buffer> <silent> R  :call QFixHowmCmd_R()<CR>
...
function! QFixHowmCmd_CR(mode) range
  ...
  if a:firstline == a:lastline
    ...
    let word = input("!!!Open files: Are you sure? (Y/<ESC>)")
    if word !~ '\cy'
      return
    endif
...
function! QFixHowmCmd_X(...) range
  ...
  let qflnum = GetJumpLine('', '')
  if qflnum
    let lnum = qflnum
  endif

  let mes = "!!!Delete Entry! Are you sure? (Y/<ESC>)"
  if a:0 > 0
    let mes = "!!!Move Entry! Are you sure? (Y/<ESC>)"
  endif
  let word = input(mes)
  if word =~ '\cy'
    call QFixHowmCmd_CR(0)
"    wincmd p
    call cursor(lnum, 1)
    if a:0 > 0
...
""""""""""""""""""""""""""""""
function! QFixHowmCmd_D() range
  let loop = 1
  if a:firstline != a:lastline
    let word = input("!!!Delete file(s): Are you sure? (Y/<ESC>)")
    if word !~ '\cy'
      return
    endif
    let loop = a:lastline - a:firstline + 1
  ...
  for index in range(1, loop)
    let file = QFixGet('file')
    if a:firstline == a:lastline
      let word = input("!!!Delete : ".file.". Are you sure? (Y/<ESC>)")
    endif
    if word =~ '\cy'
      let prevPath = getcwd()
      let prevPath = escape(prevPath, ' ')
...
""""""""""""""""""""""""""""""
function! QFixHowmCmd_R() range
  let loop = 1
  if a:firstline != a:lastline
    let word = input("!!!Remove to (~howm_dir) : Are you sure? (Y/<ESC>)")
    if word !~ '\cy'
      return
    endif
    let loop = a:lastline - a:firstline + 1
  ...
  for index in range(1, loop)
    let file = QFixGet('file')
    if a:firstline == a:lastline
      let word = input("!!!Remove to (~howm_dir) : ".file.". Are you sure? (Y/<ESC>)")
    endif
    if word =~ '\cy'
...
function! QFixHowmSaveMru(write)
  ...
  if l:write == -1
    ...
    let n = len(mqf)
    if len(qf) == n && qf[n-1]['text'] == mqf[n-1]['text']
...
function! QFixHowmHelp()
  silent! exec 'split QFixHowm Help'
  silent! exec 'setlocal filetype='.g:QFixHowm_FileType
  setlocal buftype=nofile
...
function! s:GetPatternInEntry(file, pattern, lnum)
  ...
  if s:prevgepname != file
    "検索対象は半角なのでiconvしない
    let s:tempfilebuf = readfile(file)
...
"ファイル名を指定して作成
"
function! QFixHowmCreateNewFileWithFilename()
  if QFixHowmInit()
    return
  endif
  let hfile = strftime(g:howm_filename)
  let hfile = input('filename = ', hfile)
  if hfile == ''
    return
  endif
  let sfile = g:howm_dir.'/'.hfile
...
silent! function QFixHowmFileList(path, file)
  let list = QFixHowmGetFileList(a:path, a:file)
  ...
  "サマリーを付加
  call QFixHowmFLaddtitle(a:path, list)
  call QFixHowmShowFileList(a:path, list)
endfunction
