...
"  Last Modified: 2009-10-14 22:48
"        Version: 1.57
"================================================================================
...
"
" g,e    Grep 通常のgrep
" g,f    FGrep 正規表現を使わないで検索
" g,v    VGrep vimgrepでgrep
" g,b    BGrep 現在開いている全てのバッファのみを対象にgrep (無名バッファは検索できない)
"
...
"ユーザ呼び出し用コマンド
""""""""""""""""""""""""""""""
command! -nargs=? -bang Grep    call UGrep('grep', <q-args>, <bang>0, 0)
command! -nargs=? -bang FGrep   call UGrep('grep -F', <q-args>, <bang>0, 0)
command! -nargs=? -bang Vimgrep call UGrep('vimgrep', <q-args>, <bang>0, 0)
command! -nargs=? -bang VGrep   call UGrep('vimgrep', <q-args>, <bang>0, 0)
...
command! -nargs=? -bang Grepadd    call UGrep('grep', <q-args>, <bang>0, 1)
command! -nargs=? -bang FGrepadd   call UGrep('grep -F', <q-args>, <bang>0, 1)
command! -nargs=? -bang VGrepadd   call UGrep('vimgrep', <q-args>, <bang>0, 1)
command! -nargs=? -bang Vimgrepadd call UGrep('vimgrep', <q-args>, <bang>0, 1)
...
function! QFixCopen(cmd, mode)
  let cmd = ''
  let num = matchstr(a:cmd, '\d\+')
  let cmd = strpart(a:cmd, strlen(num))
  if num > 0
    CloseQFixWin
    let g:QFix_Height = num
  endif
  if cmd != ''
    CloseQFixWin
  else
...
"  endif
"  set winfixheight
  if g:QFix_CopenCmd =~ 'vertical' && exists('g:QFix_Width')
    exec 'set winwidth='.g:QFix_Width
  endif
  set winfixwidth
